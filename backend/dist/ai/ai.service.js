...
let AiService = AiService_1 = class AiService {
    ...
    async generateSpeech(text, languageCode = 'en-IN') {
        const apiKey = this.sarvamApiKey;
        if (!apiKey) {
    ...
    async translateText(text, sourceLang, targetLang = 'en-IN') {
        const apiKey = this.sarvamApiKey;
        if (!apiKey) {
    ...
    async answerQuestion(meetingTitle, transcriptLines, question) {
    ...
    getMockQuestionAnswer(question) {
    ...
    async summarizeMeeting(transcriptLines) {
        const fullTranscript = transcriptLines.join('\n');
    ...
    async extractActionItems(transcriptLines) {
        const fullTranscript = transcriptLines.join('\n');
        if (!fullTranscript.trim()) {
            return [];
        }
    ...
    async detectRisks(transcriptLines) {
        const fullTranscript = transcriptLines.join('\n');
        if (!fullTranscript.trim()) {
            return [];
        }
    ...
    async summarizeChannel(chatLines, meetingSummaries) {
    ...
    async analyzeMeetingAnalytics(transcriptLines) {
        const fullTranscript = transcriptLines.join('\n');
    ...
    getMockSummary(transcriptLines) {
    ...
    getMockActionItems(transcriptLines) {
        const actions = [];
        const fullText = transcriptLines.join(' ').toLowerCase();
        for (const line of transcriptLines) {
            ...
            if (cleanLine.includes('will') || cleanLine.includes('should') || cleanLine.includes('need to') || cleanLine.includes('task')) {
                ...
                if (cleanLine.includes('john'))
                    assigneeName = 'John Doe';
                else if (cleanLine.includes('alice'))
                    assigneeName = 'Alice Smith';
                else if (cleanLine.includes('bob'))
        ...
        if (actions.length === 0) {
            actions.push({ text: 'Set up Prisma database schema and migrations', assigneeName: 'Alice Smith', dueDate: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] }, { text: 'Create real-time Socket.io gateway in backend', assigneeName: 'John Doe', dueDate: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] }, { text: 'Design Dashboard and Meeting Room components in React', assigneeName: 'Bob Johnson', dueDate: new Date(Date.now() + 4 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] });
        }
        return actions;
    }
    getMockRisks(transcriptLines) {
        const risks = [];
        const fullText = transcriptLines.join(' ').toLowerCase();
    ...
    getMockAnalytics(transcriptLines, durationSeconds, totalWords) {
        const talkTime = {};
        const speakerSent = {};
        const speakerTurns = {};
        for (const line of transcriptLines) {
