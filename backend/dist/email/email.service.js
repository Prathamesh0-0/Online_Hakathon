...
let EmailService = EmailService_1 = class EmailService {
    ...
    sendMockEmail(to, subject, joinLink) {
    ...
    async sendBulkInvites(emails, meetingData) {
        const results = await Promise.allSettled(emails.map((email) => this.sendMeetingInvite({ ...meetingData, recipientEmail: email })));
        const failed = [];
        let sent = 0;
